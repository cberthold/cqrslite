...
namespace Security.BoundedContext.Domain.Api.Services
{
    public class ApiService : IApiService
    {
        ...
        public ResourceActionEntity FindResourceAction(ResourceActionId resourceActionId)
        {
            var aggregate = LoadService(resourceActionId.ApiIdValue);

            if (aggregate == null)
                throw new DomainException($"Unable to find API service {resourceActionId.ApiIdValue}");
